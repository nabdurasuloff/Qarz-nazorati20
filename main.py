...
class Sidebar(tk.Frame):
    ...
    def __init__(self, parent, on_select, items):
        super().__init__(parent, bg=NAVY_DARK, width=232)
        ...
        tk.Label(title, text="🏦  Qarz Nazorat", bg=NAVY_DARK, fg=WHITE,
                 font=('Segoe UI', 13, 'bold')).pack(anchor='w', padx=20, pady=(16, 0))
        tk.Label(title, text="Talabnoma Tizimi", bg=NAVY_DARK, fg=GOLD,
                 font=('Segoe UI', 9)).pack(anchor='w', padx=20, pady=(1, 0))

        tk.Frame(self, bg='#2E3B7A', height=1).pack(fill='x')

        canvas = tk.Canvas(self, bg=NAVY_DARK, highlightthickness=0, width=232)
        vscroll = tk.Scrollbar(self, orient='vertical', command=canvas.yview)
        inner = tk.Frame(canvas, bg=NAVY_DARK)
        ...
        footer = tk.Frame(self, bg=NAVY_DARK)
        footer.pack(side='bottom', fill='x', pady=14)
        tk.Label(footer, text="Agrobank ATB — Boyovut", bg=NAVY_DARK, fg='#5B6BA8',
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.tab_talabnoma_sozlamalar = SozlamalarGuruh(
            ...
            TALABNOMA_SOZLAMA_FIELDS, sections=['xat_shablon'])
        ...
        # ---- MIB ijro harakatlari (Jarayondagilar / Harakatsizlar / Sozlamalar) ----
...
class MibTransferDialog(tk.Toplevel):
    ...
    def __init__(self, parent, mijoz_nomi, sud_ish_raqami):
        ...
        ttk.Label(self, text=f"Sud ish raqami (solishtirish uchun): {sud_ish_raqami}",
                  style='Sub.TLabel', wraplength=440).pack(anchor='w', padx=16, pady=(0, 14))
        ...
        ttk.Label(self, text="Ijro varaqasi (PDF):", style='Sub.TLabel').pack(
    ...
    def confirm(self):
        ...
        if not self.ijro_varaqasi_yol:
            messagebox.showerror("Xato", "Ijro varaqasi (PDF) faylini tanlang.")
            return
        self.result = (ish_raqami, sana, self.ijro_varaqasi_yol)
        self.destroy()
...
class SudBazaTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        cols = ('anketa', 'mijoz', 'turi', 'sud_nomi', 'jami_qarz', 'kafil_garov',
                'olib_kelingan', 'muddat_holati')
        self.tree = ttk.Treeview(self, columns=cols, show='headings', height=18)
        headings = {'anketa': 'Anketa №', 'mijoz': 'Mijoz', 'turi': 'Turi', 'sud_nomi': 'Sud turi',
                    'jami_qarz': 'Qarzdorlik', 'kafil_garov': "Kafil / Garov",
                    'olib_kelingan': 'Palatadan kelgan', 'muddat_holati': 'Sudga topshirish muddati'}
        widths = {'anketa': 85, 'mijoz': 180, 'turi': 65, 'sud_nomi': 90, 'jami_qarz': 100,
                  'kafil_garov': 200, 'olib_kelingan': 105, 'muddat_holati': 150}
        for c in cols:
            self.tree.heading(c, text=headings[c])
            self.tree.column(c, width=widths[c])
        self.tree.tag_configure('otgan', foreground=ERR)
    ...
    def refresh(self):
        ...
        for x in xatlar:
            ...
            self.tree.insert('', 'end', iid=iid, values=(
                ...
                f"{jami:,.0f}".replace(',', ' '), kafil_garov_label, olib_kelingan, muddat_label
            ), tags=(tag,))
...
class MibBazaTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        cols = ('anketa', 'mijoz', 'turi', 'jami_qarz', 'mib_holati', 'mib_ish_raqami',
                'oxirgi_harakat', 'harakat_holati', 'yigma_jild', 'nazorat')
        self.tree = ttk.Treeview(self, columns=cols, show='headings', height=18)
        headings = {'anketa': 'Anketa №', 'mijoz': 'Mijoz', 'turi': 'Turi', 'jami_qarz': 'Qarzdorlik',
                    ...
                    'yigma_jild': "Yig'ma jild", 'nazorat': 'Nazorat holati'}
        widths = {'anketa': 80, 'mijoz': 160, 'turi': 60, 'jami_qarz': 95, 'mib_holati': 85,
                  'mib_ish_raqami': 85, 'oxirgi_harakat': 100, 'harakat_holati': 120, 'yigma_jild': 75,
                  'nazorat': 220}
        for c in cols:
            self.tree.heading(c, text=headings[c])
            self.tree.column(c, width=widths[c])
    ...
    def refresh(self):
        ...
        for x in faol:
            ...
            self.tree.insert('', 'end', iid=iid, values=(
                ...
                oxirgi_sana or '', harakat_holati, jild_label, nazorat_label
            ), tags=(tag,))
            self._rows_cache[iid] = {'xat': x, 'portfel': prow}
    ...
    def mark_otkazildi(self):
        ...
        self.wait_window(dlg)
        if dlg.result:
            ish_raqami, sana, pdf_src = dlg.result
            ...
            # 1) Ijro varaqasini kunlik papkaga saqlaymiz (avvalgidek)
            dest = fayl_nusxala(pdf_src, bugungi_papka('Ijro varaqa'),
                                 prefiks='ijro_varaqasi_', mijoz_nomi=xat['mijoz_nomi'])
            db.mark_mib_otkazildi(xat['id'], ish_raqami, sana, dest, mib_ijro_summasi=mib_summasi)

            # 2) Ish uchun HAQIQIY yig'ma jild papkasini ochamiz va ijro
            #    varaqasi nusxasini ham shu yerga qo'shamiz
            jild_papka = yigma_jild_papka_yarat(xat['mijoz_nomi'], xat['anketa_raqami'], ish_raqami)
            fayl_nusxala(pdf_src, jild_papka, prefiks='01_Ijro_varaqasi_')
            ...
            # 4) Agar kafil/garov mavjud bo'lsa, ular bo'yicha ham MIB harakati
    ...
    def export_jarayondagilar_excel(self):
        ...
        for item in royxat:
            ...
            rows.append([
                xat['anketa_raqami'], item['pinfl'], xat['mijoz_nomi'], xat['mijoz_turi'],
        ...
        headers = ['Anketa raqami', 'PINFL', 'F.I.Sh / Nomi', 'Turi', 'Qarzdorlik (so\'m)',
    ...
    def add_legacy(self):
        ...
        self.wait_window(dlg)
        if dlg.result:
            prow, ish_raqami, sana, pdf_src = dlg.result
            dest = None
            if pdf_src:
                dest = fayl_nusxala(pdf_src, bugungi_papka('Ijro varaqa'),
...
class DavoArizaTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        cols = ('anketa', 'mijoz', 'turi', 'jami_qarz', 'yuborilgan', 'davo_holati',
        ...
        widths = {'anketa': 85, 'mijoz': 170, 'turi': 65, 'jami_qarz': 95,
                  'yuborilgan': 90, 'davo_holati': 130, 'ish_raqami': 85, 'taminot': 55,
                  'tavsiya': 160, 'farq_holati': 150}
        for c in cols:
            self.tree.heading(c, text=headings[c])
            self.tree.column(c, width=widths[c])
        self.tree.tag_configure('otgan', foreground=ERR)
    ...
    def refresh(self):
        ...
        for x in xatlar:
            ...
            self.tree.insert('', 'end', iid=iid, values=(
                x['anketa_raqami'], x['mijoz_nomi'], x['mijoz_turi'],
                f"{jami:,.0f}".replace(',', ' '), yuborilgan, davo_holati, ish_raqami,
        ...
        if dlg.result:
            ...
            if qoshimcha_kerak:
                ...
                messagebox.showwarning(
                    ...
                    "Davo ariza 'Olib kelindi' deb belgilandi.\n\n"
    ...
    def export_olib_kelinganlar(self):
        ...
        for item in royxat:
            xat = item['xat']
            rows.append([
                xat['anketa_raqami'], xat['mijoz_nomi'], item['pinfl'],
        ...
        headers = ['Anketa raqami', 'Mijoz', 'PINFL', 'Ish raqami', 'Chiqqan sana',
...
class SudMibJarayonidaTab(ttk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        ttk.Label(self, text="Sudga topshirilgan barcha ishlar — MIBga hali o'tkazilmaganlar "
                              ...
                              "o'tkazilganlar shu yerda ko'rinadi.",
                  style='Sub.TLabel', wraplength=1000, justify='left').pack(anchor='w', padx=20, pady=(0, 10))
        ttk.Button(self, text="🔄 Yangilash", style='Accent.TButton', command=self.refresh).pack(
            anchor='w', padx=20)

        cols = ('anketa', 'mijoz', 'turi', 'jami_qarz', 'sud_ish_raqami', 'sud_sana', 'mib_holati')
        self.tree = ttk.Treeview(self, columns=cols, show='headings', height=18)
        headings = {'anketa': 'Anketa №', 'mijoz': 'Mijoz', 'turi': 'Turi', 'jami_qarz': 'Qarzdorlik',
                    'sud_ish_raqami': 'Sud ish raqami', 'sud_sana': 'Sudga topshirilgan',
                    'mib_holati': 'MIB holati'}
        widths = {'anketa': 85, 'mijoz': 220, 'turi': 70, 'jami_qarz': 110, 'sud_ish_raqami': 110,
                  'sud_sana': 120, 'mib_holati': 140}
        for c in cols:
            self.tree.heading(c, text=headings[c])
            self.tree.column(c, width=widths[c])
    ...
    def refresh(self):
        ...
        for x in rows:
            ...
            self.tree.insert('', 'end', values=(
                xd['anketa_raqami'], xd['mijoz_nomi'], xd['mijoz_turi'],
                f"{jami:,.0f}".replace(',', ' '), xd.get('sud_ish_raqami', '') or '',
