...
def get_umumiy_tahlil():
    ...
    umumiy = conn.execute('SELECT COUNT(*) soni, COALESCE(SUM(ead),0) ead FROM portfel').fetchone()

    turi_rows = conn.execute('''
        SELECT mijoz_turi, COUNT(*) soni, COALESCE(SUM(ead),0) ead FROM portfel GROUP BY mijoz_turi
    ''').fetchall()
    ...
    tarmoq_rows = conn.execute('''
        ...
        FROM portfel GROUP BY tarmoq ORDER BY ead DESC
    ''').fetchall()

    stage_rows = conn.execute('''
        SELECT COALESCE(NULLIF(TRIM(stage), ''), "Noma'lum") AS stage,
               COUNT(*) AS soni, COALESCE(SUM(ead),0) AS ead
        FROM portfel GROUP BY stage
    ''').fetchall()
    conn.close()
...
def create_legacy_mib_xat(portfel_id, anketa_raqami, mijoz_nomi, mijoz_turi,
    ...
    xat_turi = 'Talabnoma' if mijoz_turi == 'yuridik' else 'Ogohlantirish'
    ...
    cur.execute('''
        INSERT INTO xatlar (portfel_id, anketa_raqami, mijoz_nomi, mijoz_turi, xat_turi,
                             ...
                             sud_holati, sud_topshirilgan_sana,
                             mib_holati, mib_ish_raqami, mib_otkazilgan_sana, ijro_varaqasi_fayl,
                             mib_ijro_summasi)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'yuborildi', ?, '', ?, 'olib_kelindi',
                'topshirildi', ?, 'otkazildi', ?, ?, ?, ?)
    ''', (portfel_id, anketa_raqami, mijoz_nomi, mijoz_turi, xat_turi,
          now_iso, now_iso, now_iso, now_iso, mib_sana, mib_ish_raqami, mib_sana, ijro_varaqasi_fayl,
          jami_qarz))
    conn.commit()
    xid = cur.lastrowid
    conn.close()
...
def get_mib_faol_royxat():
    """MIBga o'tkazilgan (undirish jarayonidagi) barcha mijozlar ro'yxati."""
    conn = get_conn()
    rows = conn.execute('''
        SELECT * FROM xatlar WHERE mib_holati='otkazildi'
...
def get_mib_faol_royxat_toliq():
    ...
    for x in faol:
        ...
        if not amallar:
            natija.append({'xat': x, 'pinfl': pinfl, 'jami_qarz': jami_qarz, 'amal': None})
        else:
            for amal in amallar:
                natija.append({'xat': x, 'pinfl': pinfl, 'jami_qarz': jami_qarz, 'amal': amal})
    return natija
...
def get_ish_kuni_rejasi():
    ...
    xat_bajarildi = conn.execute(
        "SELECT COUNT(*) c FROM xatlar WHERE yuborilgan_sana LIKE ?", (bugun_iso + '%',)
    ).fetchone()['c']
    davo_bajarildi = conn.execute(
        "SELECT COUNT(*) c FROM xatlar WHERE davo_ariza_sana LIKE ?", (bugun_iso + '%',)
    ).fetchone()['c']
...
def get_95413_royxati():
    ...
    portfel_rows = conn.execute(
        "SELECT * FROM portfel WHERE balans_95413 IS NOT NULL AND balans_95413 != 0"
    ).fetchall()
